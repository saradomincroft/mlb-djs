...
def add_dj():
    ...
    while True:
        genre_title = input("Enter Genre: ").strip().lower()
        if not genre_title:
            print("Genre cannot be blank. Please enter a valid genre.")
            continue

        mapped_genre_title = genre_mapping.get(genre_title, genre_title).title()
        ...
        while True:
            subgenre_title = input(f"Enter Subgenre of {mapped_genre_title} that {name} plays: ").strip().lower()
            if not subgenre_title:
                print("Subgenre cannot be blank. Please enter a valid subgenre.")
                continue

            mapped_subgenre_title = genre_mapping.get(subgenre_title, subgenre_title).title()
    ...
    while True:
        venue_name = input("Enter a venue where the DJ has played: ").strip().title()
        if not venue_name:
    ...
    session.commit()
    print(f"{name} added successfully.") # This not working? :(
